                ...
                periods.forEach(period => {
                  ...
                  const convertedClients = cleanedSalesData.filter(sale => {
                    ...
                    if (!saleDateAfterVisit) {
                      conversionReason = 'Purchase date not after first visit';
                    } else if (!notExcludedCategory) {
                      conversionReason = `Excluded category: ${sale['Category']}`;
                    } else if (!not2For1Item) {
                      conversionReason = `Excluded item (2 for 1): ${sale['Item']}`;
                    } else if (!hasSaleValue) {
                      conversionReason = 'No sale value or zero value';
                    } else if (!notRefunded) {
                      conversionReason = 'Purchase was refunded';
                    } else {
                      conversionReason = `Converted with ${sale['Item']} for ${saleValue}`;
                    }
